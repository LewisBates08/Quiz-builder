...
class App(tk.Tk):
    def __init__(self):
        ...
        #Log Out Button
        log=tk.Button(self.sidebar,text="Log Out",command=lambda:self.logOut(),background="#183EC6",)
        log.pack(pady=28)
...
class Login(tk.Frame):
    def __init__(self,parent,controller):
        ...
        self.label=tk.Label(self.container, text= "Login", font=("Arial",20)).grid(row=0, column=1)
    ...
    def loginbutton(self,controller):
        if self.username.get()=="LEWIS" and self.password.get()=="171008":
            ...
            self.loginmessage=tk.Label(self.container,text="Invalid login details",font=("Helvetica,18"))
